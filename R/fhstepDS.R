fhstepDS<-function(y,des,m,c,prvm) {
  ...
  if(m==0) {
    ...
    indx<-rep(0,m)
    for (i in 1:m) {
      indx[i]<-match(lin[i],repl.tab[, 1], nomatch=0)  ### the problem seems to be m=4 the number of terms in previous model  not the number of cols in subdes
    }
    quad<-rep('A',m)
    for (i in 1:m) {
      quad[i]<-lin[i]
